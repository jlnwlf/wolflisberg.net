...
RUN cd ${HOME} ; git clone https://github.com/yyuu/pyenv.git ${HOME}/.pyenv && echo 'export PYENV_ROOT="$HOME/.pyenv"' >> ${HOME}/.bashrc && echo 'export PATH="$PYENV_ROOT/bin:$PATH"' >> ${HOME}/.bashrc && echo 'eval "$(pyenv init -)"' >> ${HOME}/.bashrc && export PYENV_ROOT="$HOME/.pyenv" && export PATH="$PYENV_ROOT/bin:$PATH" && eval "$(pyenv init -)" && pyenv install 2.7.11 && pyenv install 3.5.1 && pyenv global 2.7.11 3.5.1 && pip install -U pip
